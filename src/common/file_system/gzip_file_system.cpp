...
bool MiniZStreamWrapper::read(StreamData& sd) {
    if (sd.refresh) {
        ...
        if (gzipHdr[3] & GZipFileSystem::GZIP_FLAG_NAME) {
            ...
            RYU_ASSERT(bodyPtr - sd.inputBufStart < GZipFileSystem::GZIP_HEADER_MAXSIZE);
        }
    ...
    RYU_ASSERT(sd.outputBufEnd + mzStreamPtr->avail_out == sd.outputBuf.get() + sd.outputBufSize);
