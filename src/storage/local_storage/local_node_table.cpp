...
bool LocalNodeTable::insert(Transaction* transaction, TableInsertState& insertState) {
    ...
    RYU_ASSERT(nodeInsertState.pkVector.state->getSelVector().getSelSize() == 1);
...
bool LocalNodeTable::update(Transaction* transaction, TableUpdateState& updateState) {
    ...
    RYU_ASSERT(nodeUpdateState.columnID != table.cast<NodeTable>().getPKColumnID());
    RYU_ASSERT(offset >= startOffset);
