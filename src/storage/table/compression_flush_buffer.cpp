...
ColumnChunkMetadata uncompressedFlushBuffer(std::span<const uint8_t> buffer, FileHandle* dataFH,
    ...
    RYU_ASSERT(buffer.size_bytes() <= entry.numPages * RYU_PAGE_SIZE);
...
ColumnChunkMetadata CompressedFlushBuffer::operator()(std::span<const uint8_t> buffer,
    ...
    RYU_ASSERT(numValuesPerPage * entry.numPages >= metadata.numValues);
    while (valuesRemaining > 0) {
        ...
        RYU_ASSERT(numPages < entry.numPages);
        RYU_ASSERT(dataFH->getNumPages() >= entry.startPageIdx + numPages);
...
std::pair<std::unique_ptr<uint8_t[]>, uint64_t> flushCompressedFloats(const CompressionAlg& alg,
    ...
    RYU_ASSERT(floatMetadata->exceptionCapacity >= floatMetadata->exceptionCount);

    auto valuesRemaining = metadata.numValues;
    RYU_ASSERT(valuesRemaining <= buffer.size_bytes() / sizeof(T));
    ...
    RYU_ASSERT(totalExceptionCount == floatMetadata->exceptionCount);
...
void flushALPExceptions(std::span<const uint8_t> exceptionBuffer, FileHandle* dataFH,
    ...
    RYU_ASSERT(exceptionStartPageIdx + preExceptionMetadata.getNumPages() <= dataFH->getNumPages());
