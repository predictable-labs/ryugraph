...
public:
    ...
    [[nodiscard]] T* allocate(const std::size_t size) {
        RYU_ASSERT_UNCONDITIONAL(mm != nullptr);
        RYU_ASSERT_UNCONDITIONAL(size > 0);
        RYU_ASSERT_UNCONDITIONAL(size <= std::numeric_limits<std::size_t>::max() / sizeof(T));

        auto buffer = mm->mallocBuffer(false, size * sizeof(T));
        auto p = reinterpret_cast<T*>(buffer.data());

        // Ensure proper alignment
        RYU_ASSERT_UNCONDITIONAL(reinterpret_cast<std::uintptr_t>(p) % alignof(T) == 0);
    ...
    void deallocate(T* p, const std::size_t size) noexcept {
        RYU_ASSERT_UNCONDITIONAL(mm != nullptr);
        RYU_ASSERT_UNCONDITIONAL(p != nullptr);
        RYU_ASSERT_UNCONDITIONAL(size > 0);
